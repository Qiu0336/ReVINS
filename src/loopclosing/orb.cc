...
void _ExtractorNode::DivideNode(_ExtractorNode &n1, _ExtractorNode &n2, _ExtractorNode &n3, _ExtractorNode &n4) {
...
static bool compareNodes(std::pair<int, _ExtractorNode *> &e1, std::pair<int, _ExtractorNode *> &e2)
{
...
ORB_Extractor::ORB_Extractor(int _n_features, float _scale_factor, int _n_levels, int _initial_threshold_fast,
                             int _min_threshold_fast)
    : n_features(_n_features), scale_factor(_scale_factor), n_levels(_n_levels),
      initial_threshold_fast(_initial_threshold_fast), min_threshold_fast(_min_threshold_fast)
{
    mvscale_factor.resize(n_levels);
    mvscale_factor[0] = 1.0f;
    for(int i = 1; i < n_levels; i++)
    {
        mvscale_factor[i] = mvscale_factor[i - 1] * scale_factor;
    }

    mvInvscale_factor.resize(n_levels);
    for (int i = 0; i < n_levels; i++)
    {
        mvInvscale_factor[i] = 1.0f / mvscale_factor[i];
    }

    mvImagePyramid.resize(n_levels);

    mn_featuresPerLevel.resize(n_levels);
    float factor = 1.0f / scale_factor;
    float nDesiredFeaturesPerScale = n_features * (1 - factor) / (1 - (float)pow((double)factor, (double)n_levels));


    int sumFeatures = 0;
    for (int level = 0; level < n_levels - 1; level++)
    {
        mn_featuresPerLevel[level] = cvRound(nDesiredFeaturesPerScale);
        sumFeatures += mn_featuresPerLevel[level];
        nDesiredFeaturesPerScale *= factor;
    }
    mn_featuresPerLevel[n_levels - 1] = std::max(n_features - sumFeatures, 0);
                               ...
                               const std::vector<cv::Point>& pattern)
{
    ...
    for(size_t i = 0; i < keypoints.size(); i++)
    {
        ...
        const cv::Point* p_pattern = &pattern[0];
        uchar* desc = descriptors.ptr((int)i);

        #define GET_VALUE(idx) \
        center[cvRound(p_pattern[idx].x*b + p_pattern[idx].y*a)*step + \
               cvRound(p_pattern[idx].x*a - p_pattern[idx].y*b)]

        for(int j = 0; j < 32; ++j, p_pattern += 16)
        {
            int t0, t1, val;
...
void ORB_Extractor::Detect(cv::InputArray _image, std::vector<cv::KeyPoint> &_keypoints, cv::OutputArray _descriptors)
{
    assert(!_image.empty());
    ...
    for(int level = 0; level < n_levels; ++level)
    {
        ...
        float scale = mvscale_factor[level];
        int i = 0;
...
void ORB_Extractor::ComputePyramid(cv::Mat image)
{
    for(int level = 0; level < n_levels; ++level)
    {
        float scale = mvInvscale_factor[level];
...
void ORB_Extractor::ComputeKeyPointsOctTree(std::vector<std::vector<cv::KeyPoint>> &allKeypoints)
{
    ...
    for(int level = 0; level < n_levels; ++level)
    {
        ...
        vToDistributeKeys.reserve(n_features * 10);
        ...
        for(int i = 0; i < nRows; i++)
        {
            ...
            for(int j = 0; j < nCols; j++)
            {
                ...
                FAST(mvImagePyramid[level].rowRange(iniY, maxY).colRange(iniX, maxX), vKeysCell, initial_threshold_fast, true);

                if(vKeysCell.empty())
                {
                    FAST(mvImagePyramid[level].rowRange(iniY, maxY).colRange(iniX, maxX), vKeysCell, min_threshold_fast, true);
                }
        ...
        std::vector<cv::KeyPoint> &keypoints = allKeypoints[level];
        keypoints.reserve(n_features);

        keypoints = DistributeOctTree(vToDistributeKeys, minBorderX, maxBorderX, minBorderY, maxBorderY,
                                      mn_featuresPerLevel[level]);

        const int scaledPatchSize = patch_size * mvscale_factor[level];
...
std::vector<cv::KeyPoint> ORB_Extractor::DistributeOctTree(const std::vector<cv::KeyPoint> &vToDistributeKeys,
                                                      const int &minX, const int &maxX, const int &minY,
                                                      const int &maxY, const int &N)
{
    // Compute how many initial nodes
    int tmp = round(static_cast<float>(maxX - minX) / (maxY - minY));
    const int nIni = tmp > 0 ? tmp : 1;
    const float hX = static_cast<float>(maxX - minX) / nIni;

    std::list<_ExtractorNode> lNodes;

    std::vector<_ExtractorNode *> vpIniNodes;
    ...
    std::list<_ExtractorNode>::iterator lit = lNodes.begin();
    ...
    std::vector<std::pair<int, _ExtractorNode *>> vSizeAndPointerToNode;
    ...
    while(!bFinish)
    {
        ...
        else if(((int)lNodes.size() + nToExpand * 3) > N)
        {
            while(!bFinish)
            {
                ...
                std::vector<std::pair<int, _ExtractorNode *>> vPrevSizeAndPointerToNode = vSizeAndPointerToNode;
                vSizeAndPointerToNode.clear();
    ...
    vResultKeys.reserve(n_features);
    for(std::list<_ExtractorNode>::iterator lit = lNodes.begin(); lit != lNodes.end(); lit++)
    {
...
static void computeKeyPoints(const Mat& imagePyramid,
                             ...
                             int edgeThreshold, int patchSize, int fastThreshold)
{
    ...
    float factor = (float)(1.0 / scaleFactor);// 0.8333
    ...
    for(level = 0; level < nlevels; level++ )
    {
        ...
        FAST(img, keypoints, fastThreshold, true);

        // Remove keypoints very close to the border
        KeyPointsFilter::runByImageBorder(keypoints, img.size(), edgeThreshold);

        // keep best corners, 2*featuresNum
...
computeOrbDescriptors( const Mat& imagePyramid, const std::vector<Rect>& layerInfo,
                       ...
                       Mat& descriptors, const std::vector<Point>& _pattern)
{
...
void detectAndCompute( InputArray _image, std::vector<KeyPoint>& keypoints, OutputArray _descriptors)
{
    ...
    {
        float scale = (float)std::pow(scaleFactor, level);
    ...
    for(int level = 0; level < nLevels; ++level)
    {
        ...
        Mat currImg = extImg(Rect(border, border, sz.width, sz.height));
