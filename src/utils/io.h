...
struct imu_data_t
{
    ...
    {
        return Eigen::Vector3d(w_x, w_y, w_z);
    }
...
bool IsSimu(timestamp_t t1, timestamp_t t2);

template<typename T, typename T_iter>
T_iter Move_Iterator(T_iter i, T_iter j, timestamp_t _t)
{
    ...
    T_iter it_ = std::prev(it);
    if ((it->timestamp - t) > (t - it_->timestamp)) return it_;
    else return it;
}

template<typename T, typename T_iter>
T_iter Move_Iterator(T_iter i, T_iter j, timestamp_t _t, bool &state)
{
    timestamp_t t = i->timestamp + _t;
    T_iter it = std::upper_bound(i, j, t,
        [](const timestamp_t lhs, const T &rhs)
...
template<typename T, typename T_iter>
T_iter Find_Iterator(T_iter i, T_iter j, timestamp_t t)
{
    T_iter it = std::upper_bound(i, j, t,
        [](const timestamp_t lhs, const T &rhs)
        {return lhs < rhs.timestamp;});
...
template<typename T, typename T_iter>
T_iter Find_Iterator(T_iter i, T_iter j, timestamp_t t, bool &state)
{
    T_iter it = std::upper_bound(i, j, t,
        [](const timestamp_t lhs, const T &rhs)
        {return lhs < rhs.timestamp;});
